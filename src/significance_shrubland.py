    ...
    for raster in raster_fps:
        ...
        with rasterio.open(raster) as r:
            try:
                ...
                pos_sig_trend = abs(np.where(sigtrend > 0, sigtrend, 0)).mean()
                neg_sig_trend = abs(np.where(sigtrend < 0, sigtrend, 0)).mean()
                net_sig_trend = abs(sigtrend).mean()
                ...
                results.append((geometry, tile_id, country, region, precip, pos_sig_diff, 
                                neg_sig_diff, net_sig_diff, pos_sig_trend, neg_sig_trend, 
                                net_sig_trend, summed_pos_diff_m2, summed_neg_diff_m2,
                                pos_pixels, neg_pixels, total_pixels))
                
                count =+ 1
                return (pd.DataFrame(results, columns=["geometry", "tile_id", "country", "region", "precip", 
                                                       "mean_pos_sig_diff", "mean_neg_sig_diff", "mean_net_sig_diff", "mean_pos_trend", 
                                                       "mean_neg_trend", "mean_net_sig_trend", 'pos_diff_m2', 'neg_diff_m2',
...
if __name__ == "__main__":
    ...
    prediction_suffix = "silver_sweep_9_ag"

    # calculate individual tree stats
    data = tree_stats(shape, landsat_prediction_dir, prediction_suffix, False, True)
    dfs, total = data
    df = pd.concat(dfs)
    df.to_csv(f"significant_trends_ls789_m2_cropland.csv", index=False)
    gdf = gpd.GeoDataFrame(df, geometry="geometry")
    gdf.to_file("significant_trends_ls789_m2_cropland.gpkg", driver="GPKG")
